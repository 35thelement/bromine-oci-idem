...
,
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_oci.compute_management_is_present:
    ...
    if resource_id:
        # Possible parameters: **{}
        before = await hub.exec.oci.compute_management.get(
            ctx,
            name=name,
        ...
        result["comment"].append(f"'oci.compute_management: {name}' already exists")
        ...
        if ctx.test:
            ...
            if result["result"]:
                result["comment"].append(f"Created 'oci.compute_management: {name}'")
    ...
    # Possible parameters: **{}
    after = await hub.exec.oci.compute_management.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
,
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_oci.compute_management_is_absent:
    ...
    if not resource_id:
        result["comment"].append(f"'oci.compute_management: {name}' already absent")
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result["comment"] = f"Would delete oci.compute_management: {name}"
            return result

        delete_ret = await hub.exec.oci.compute_management.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(f"Deleted oci.compute_management: {name}")
        else:
