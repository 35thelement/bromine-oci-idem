"""States module for managing Virtual Networks. """
...
async def present(
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_oci.virtual_network_is_present:
              oci.oci.virtual_network.present: []
    ...
    if resource_id:
        # Possible parameters: **{}
        before = await hub.exec.oci.virtual_network.get(
            ctx,
            name=name,
        ...
        result["comment"].append(f"'oci.virtual_network: {name}' already exists")
        ...
        if bool(changes.get("new")):
            if ctx.test:
                ...
                result["comment"].append(f"Would update oci.virtual_network: {name}")
                return result
            else:
                # Update the resource
                update_ret = await hub.exec.oci.virtual_network.update(ctx, **{})
                result["result"] = update_ret["result"]

                if result["result"]:
                    result["comment"].append(f"Updated 'oci.virtual_network: {name}'")
                else:
        ...
        if ctx.test:
            ...
            result["comment"] = (f"Would create oci.virtual_network: {name}",)
            return result
        else:
            create_ret = await hub.exec.oci.virtual_network.create(ctx, **{})
            result["result"] = create_ret["result"]

            if result["result"]:
                result["comment"].append(f"Created 'oci.virtual_network: {name}'")
    ...
    # Possible parameters: **{}
    after = await hub.exec.oci.virtual_network.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
async def absent(
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_oci.virtual_network_is_absent:
              oci.oci.virtual_network.absent: []
    ...
    if not resource_id:
        result["comment"].append(f"'oci.virtual_network: {name}' already absent")
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result["comment"] = f"Would delete oci.virtual_network: {name}"
            return result

        delete_ret = await hub.exec.oci.virtual_network.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(f"Deleted oci.virtual_network: {name}")
        else:
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Example:
        ...
        .. code-block:: bash
            ...
            $ idem describe oci.virtual_network
    """

    result = {}

    # TODO: Add other required parameters from: {}
    ret = await hub.exec.oci.virtual_network.list(ctx)

    if not ret or not ret["result"]:
        hub.log.debug(f"Could not describe oci.virtual_network {ret['comment']}")
        return result
