...
async def present(
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_oci.blockstorage_is_present:
    ...
    if resource_id:
        # Possible parameters: **{}
        before = await hub.exec.oci.blockstorage.get(
            ctx,
            name=name,
        ...
        result["comment"].append(f"'oci.blockstorage: {name}' already exists")
    ...
    # Possible parameters: **{}
    after = await hub.exec.oci.blockstorage.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
async def absent(
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_oci.blockstorage_is_absent:
    ...
    if not resource_id:
        result["comment"].append(f"'oci.blockstorage: {name}' already absent")
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result["comment"] = f"Would delete oci.blockstorage: {name}"
            return result

        delete_ret = await hub.exec.oci.blockstorage.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(f"Deleted oci.blockstorage: {name}")
        else:
